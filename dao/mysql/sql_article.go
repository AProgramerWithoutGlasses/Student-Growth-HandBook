...
func QueryArticleByIdOfPassenger(aid int) (err error, article *model.Article) {
	if err = DB.Preload("ArticlePics").Preload("ArticleTags.Tag").Preload("User").
		Select("pic").Select(" articles.ban, tag_name, articles.created_at, like_amount, collect_amount, comment_amount, article_quality").Select("headshot, name, username, class").
